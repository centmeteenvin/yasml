...
void main() {
  group('Query Container Sharing', () {
    test('Identical queries share the same container', () async {
      ...
      final sub1 = world.queryManager.subscribe(RankingsStreamQuery(), MockQueryListener());
      await world.settled;

      final createdCount1 = observer.events
      ...
      final sub2 = world.queryManager.subscribe(RankingsStreamQuery(), MockQueryListener());

      final createdCount2 = observer.events
          .whereType<QueryContainerCreatedEvent>()
          .where((e) => e.queryKey == 'Rankings')
          .length;

      expect(createdCount2, 0, reason: 'Second subscription reuses the container (0 new created)');
      ...
      expect(sub1.queryContainer.state, equals(sub2.queryContainer.state),
      ...
      final firstUnsubEventCount = observer.events
          .whereType<QueryContainerListenerRemovedEvent>()
          .where((e) => e.queryKey == 'Rankings')
          .length;

      final firstDisposeEventCount = observer.events
          .whereType<QueryContainerDisposedEvent>()
          .where((e) => e.queryKey == 'Rankings')
          .length;

      expect(firstUnsubEventCount, 1, reason: 'First unsubscribe removes a listener');
      expect(firstDisposeEventCount, 0, reason: 'Container should not dispose while second listener exists');
      ...
      final secondUnsubEventCount = observer.events
          .whereType<QueryContainerListenerRemovedEvent>()
          .where((e) => e.queryKey == 'Rankings')
          .length;

      final secondDisposeEventCount = observer.events
          .whereType<QueryContainerDisposedEvent>()
          .where((e) => e.queryKey == 'Rankings')
          .length;

      expect(secondUnsubEventCount, 1, reason: 'Second unsubscribe removes the last listener');
      expect(secondDisposeEventCount, 1,
